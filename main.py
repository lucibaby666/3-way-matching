...
class ChromaVectorDatabase:
    def __init__(self, persist_path: str = "./chroma_db_store"):
        ...
        if CHROMADB_AVAILABLE:
            try:
                ...
                print(f"✅ ChromaDB ready: {self.collection.count()} vectors")
                logger.info(f"✅ ChromaDB ready: {self.collection.count()} vectors")
    ...
    def store_precedent(self, exception: dict, decision: str, reviewer: str, comment: str = "") -> str:
        ...
        try:
            ...
            precedent_id = f"PREC-{datetime.now().strftime('%Y%m%d_%H%M%S')}-{hashlib.md5(signature.encode()).hexdigest()[:8]}"
            ...
            # Log to database using separated function
            audit_entry = {
                "audit_id": f"PREC-{datetime.now().strftime('%Y%m%d_%H%M%S')}",
            ...
            print(f"✅ Precedent stored: {precedent_id}")
    ...
    def find_similar(self, exception: dict, threshold: float = 0.98) -> dict:
        if not self.collection:
            return None
...
class SmartApprovalSystem:
    ...
    def process_exception(self, exception: dict) -> dict:
        ...
        print(f"\n🔍 Processing: {exc_type} on {item_code}")
        logger.info(f"Processing exception: {exc_type} on {item_code}")
        ...
        if precedent:
            ...
            audit_entry = {
                "audit_id": f"AUTO-{datetime.now().strftime('%Y%m%d_%H%M%S')}",
                "event_type": "AUTO_APPROVAL",
                "severity": "INFO",
                "user": "system",
                "action": f"Auto-{precedent['decision']}: {exc_type} on {item_code}",
            ...
            print(f"   ✅ AUTO-{precedent['decision']} (Score: {precedent['similarity']:.4f})")
            ...
            print(f"   ❌ NO MATCH - Routing to Human")
            logger.warning(f"   ❌ NO MATCH - Routing to Human: {exc_type} on {item_code}")
    ...
    def store_human_decision(self, exception: dict, decision: str, reviewer: str, comment: str = "") -> bool:
        exc_type = exception.get('type', 'UNKNOWN') or 'UNKNOWN'
        item_code = exception.get('item_code', 'UNKNOWN') or 'UNKNOWN'
        ...
        precedent_id = self.vector_db.store_precedent(exception, decision, reviewer, comment)
        
        if precedent_id:
            print(f"✅ Human decision stored as precedent: {precedent_id}")
            logger.info(f"✅ Human decision stored as precedent: {precedent_id}")
            return True
